...
from app.common.window_effect.window_effect import WindowEffect
...
class TextEditMenu(QMenu):
    ...
    def createActions(self):
        # 创建动作
        self.cutAct = QAction(
            QIcon('app\\resource\\Image\\text_edit_menu\\黑色剪刀.png'), '剪切', self, shortcut='Ctrl+X', triggered=self.parent().cut)
        self.copyAct = QAction(
            QIcon('app\\resource\\Image\\text_edit_menu\\黑色复制.png'), '复制', self, shortcut='Ctrl+C', triggered=self.parent().copy)
        self.pasteAct = QAction(
            QIcon('app\\resource\\Image\\text_edit_menu\\黑色粘贴.png'), '粘贴', self, shortcut='Ctrl+V', triggered=self.parent().paste)
        self.cancelAct = QAction(
            QIcon('app\\resource\\Image\\text_edit_menu\\黑色撤销.png'), '取消操作', self, shortcut='Ctrl+Z', triggered=self.parent().undo)
    ...
    def __setQss(self):
        """ 设置层叠样式 """
        with open('app\\resource\\qss\\text_edit_menu.qss', encoding='utf-8') as f:
